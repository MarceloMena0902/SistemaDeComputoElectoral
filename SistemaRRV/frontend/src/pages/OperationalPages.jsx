...
function deptNombreDesdeActa(acta) {
  const directo = normalizarDepartamento(acta.datos?.departamento)
  if (directo) return directo

  const codigo = acta.datos?.nro_acta || acta.datos?.codigo_recinto || acta.recinto_id || extraerCodigoNombre(acta.nombre)
  const mapa = {
    '1': 'Chuquisaca',
    '2': 'La Paz',
    '3': 'Cochabamba',
  ...
  return mapa[String(codigo || '').charAt(0)] || 'Pendiente'
}
...
function extraerCodigoNombre(nombre) {
  return nombre?.match(/acta_(\d{10,})/)?.[1] || nombre?.match(/SMS(?:_TEST)?_(\d{6,})/)?.[1] || null
}
...
export function MapaPage() {
  ...
  const ranking = useMemo(() => {
    const valueFor = d => {
      ...
      if (visualMode === 'actas') return d.mesas > 0 ? (d.actas / d.mesas) * 100 : 0
  ...
  const actasPct = selectedDept.mesas > 0 ? (selectedDept.actas / selectedDept.mesas) * 100 : 0
      ...
      <section className="map-main-layout">
        <div className="map-board card">
          <div className="map-board__head">
            ...
            <div className="map-tools">
              ...
              <button onClick={()=>setMapZoom(1)}>â†»</button>
            </div>
          </div>
        ...
        <aside className="map-inspector card">
          ...
          <div className="map-inspector__metrics">
            ...
            <div><label>Margen del lider</label><strong className="accent-number">+{Math.max(0, selectedDept.pct[selectedDept.leader]-24.7).toFixed(1)}<small> pp</small></strong><span>vs. 2do lugar</span></div>
          ...
          <div className="map-inspector__actions"><a className="btn btn--accent" href={detailHref}>Deep dive â†’</a><a className="btn" href="#actas">Ver actas</a></div>
        </aside>
      </section>
...
// ============================================================
// ACTAS PAGE - CON PAGINACIÃ“N REAL (50 por pÃ¡gina)
// ============================================================
export function ActasPage() {
  ...
  const formatearActaLocal = (acta) => ({
    ...
    _id: acta._id,
    nombre: acta.nombre || '-',
    ciudad: deptNombreDesdeActa(acta),
  ...
  if (cargando && actas.length === 0) {
    return <div style={{display:'flex',justifyContent:'center',alignItems:'center',height:'60vh'}}><div style={{textAlign:'center'}}><h2>ðŸ“„ Cargando actas...</h2><p style={{color:'var(--text-muted)'}}>Conectando con MongoDB...</p></div></div>
  }
    ...
    <div className="actas-page">
      ...
      <section className="card actas-console">
        <div className="actas-filters">
          ...
          <button className="btn" type="button" onClick={exportCsv}>â†“ Exportar CSV</button>
        </div>
...
export function DeptPage() {
  ...
  const [deptActas, setDeptActas] = useState([])

  useEffect(() => {
    let alive = true
    api.getActas(null, 1000, 0).then(data => {
      if (!alive) return
      const rows = (data.actas || [])
        .map(a => {
          const votos = a.votos || a.datos || {}
          const p1 = votos.partido1 || votos.p1 || votos.P1 || 0
          const p2 = votos.partido2 || votos.p2 || votos.P2 || 0
          const p3 = votos.partido3 || votos.p3 || votos.P3 || 0
          const p4 = votos.partido4 || votos.p4 || votos.P4 || 0
          return {
            id: `A-${a._id?.slice(-8) || ''}`,
            nombre: a.nombre || '-',
            dept: deptNombreDesdeActa(a),
            ciudad: a.datos?.municipio || a.datos?.recinto || deptNombreDesdeActa(a),
            recinto: a.datos?.recinto || a.datos?.codigo_recinto || a.recinto_id || a.nombre?.substring(0, 30) || 'Sin recinto',
            estado: a.estado || 'PENDIENTE',
            hora: a.fecha_recepcion ? new Date(a.fecha_recepcion).toLocaleTimeString('es-BO') : (a.fecha_procesamiento ? new Date(a.fecha_procesamiento).toLocaleTimeString('es-BO') : '-'),
            votos: { P1: p1, P2: p2, P3: p3, P4: p4 },
            total: p1 + p2 + p3 + p4,
          }
        })
  ...
  const actasPct = dept.mesas > 0 ? (dept.actas / dept.mesas) * 100 : 0

  const ciudades = Object.values(deptActas.reduce((acc, acta) => {
    const key = acta.ciudad || 'Sin localidad'
    if (!acc[key]) acc[key] = { nombre: key, actas: 0, votos: 0, votosPartido: { P1: 0, P2: 0, P3: 0, P4: 0 } }
    acc[key].actas += 1
    acc[key].votos += acta.total
    for (const id of ['P1', 'P2', 'P3', 'P4']) acc[key].votosPartido[id] += acta.votos[id] || 0
    return acc
  }, {})).map(c => ({
    ...c,
    pct: c.votos > 0 ? +(c.votosPartido[dept.leader] / c.votos * 100).toFixed(1) : 0,
  })).sort((a, b) => b.actas - a.actas).slice(0, 5)

  const recentActas = deptActas.slice(0, 10)
  const trendValues = dept.actas > 0 ? [0, Math.round(dept.actas*0.08), Math.round(dept.actas*0.18), Math.round(dept.actas*0.32), Math.round(dept.actas*0.48), Math.round(dept.actas*0.64), Math.round(dept.actas*0.78), dept.actas] : [0, 0]
  const chartW = 760; const chartH = 160; const maxTrend = Math.max(dept.actas, 1)
