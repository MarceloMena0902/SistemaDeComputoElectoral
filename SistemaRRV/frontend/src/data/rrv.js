// src/data/rrv.js - VERSIÓN COMPLETA CON DEPARTAMENTOS DINÁMICOS
...
function getDepartamentoFromActa(acta) {
  // Intentar obtener departamento de diferentes fuentes
  
  // 1. Del campo datos.departamento (OCR)
  if (acta.datos?.departamento) {
    const dept = acta.datos.departamento
    return mapNombreDept(dept)
  }
  
  // 2. Del recinto_id (SMS)
  if (acta.recinto_id) {
    return mapRecintoToDept(acta.recinto_id)
  }
  
  // 3. Del nombre del archivo (formato: acta_CODIGO_MESA.pdf)
  if (acta.nombre) {
    const match = acta.nombre.match(/acta_(\d{4})(\d{2})(\d{4})/)
    if (match) {
      return mapRecintoToDept(match[1] + match[2] + match[3])
    }
    // Intentar extraer código de departamento
    const deptMatch = acta.nombre.match(/acta_(\d{2})/)
    if (deptMatch) {
      return mapCodigoToDept(deptMatch[1])
    }
  }
...
function mapNombreDept(nombre) {
  ...
  if (n.includes('la paz')) return 'BO-L'
  if (n.includes('santa cruz')) return 'BO-S'
  if (n.includes('cochabamba')) return 'BO-C'
  if (n.includes('oruro')) return 'BO-O'
  if (n.includes('potosi') || n.includes('potosí')) return 'BO-P'
  if (n.includes('chuquisaca')) return 'BO-H'
  if (n.includes('tarija')) return 'BO-T'
  if (n.includes('beni')) return 'BO-B'
  if (n.includes('pando')) return 'BO-N'
  return null
}

function mapRecintoToDept(recintoId) {
  const codigo = String(recintoId || '')
  // Los primeros 2 dígitos del recinto indican el departamento
  // 10 = Chuquisaca, 20 = La Paz, 30 = Cochabamba, etc.
  const prefix = codigo.substring(0, 2)
  return mapCodigoToDept(prefix)
}
...
async function calcularDepartamentosDesdeActas() {
  console.log('🗺️ Calculando departamentos desde actas...')
  ...
  try {
    ...
    if (!data || !data.actas || data.actas.length === 0) {
      console.warn('⚠️ No hay actas para calcular departamentos, usando datos por defecto')
    ...
    for (const acta of data.actas) {
      ...
      if (deptId && deptStats[deptId]) {
        ...
        // Registrar mesa única
        const mesa = acta.nro_mesa || acta.datos?.nro_mesa || acta.nombre
    ...
    console.log(`🗺️ Actas asignadas: ${actasAsignadas}, sin departamento: ${actasSinDept}`)
    ...
    const deptsConDatos = deptArray.filter(d => d.actas > 0)
    
    if (deptsConDatos.length < 3) {
      console.warn('⚠️ Pocos departamentos con datos, complementando con defaults')
      return complementarConDefaults(deptArray)
    }
    
    console.log('✅ Departamentos calculados:', deptArray.map(d => `${d.name}: ${d.actas} actas, ${d.votos} votos, líder: ${d.leader}`))
    return deptArray
    
  } catch (error) {
    console.error('❌ Error calculando departamentos:', error)
    return crearDepartamentosDefault()
  }
}
...
export async function loadRealData() {
  ...
  try {
    ...
    // 3. Calcular departamentos DINÁMICAMENTE desde actas reales
    departments = await calcularDepartamentosDesdeActas()
    console.log('🗺️ Departamentos:', departments.map(d => d.name))
    
    // 4. Cargar primer lote de actas
    await cargarPrimerLote()
    
    console.log(`✅ Datos cargados: ${actas.length} actas, ${parties.length} partidos, ${departments.length} departamentos`)
