...
export default function MapBlock({
  ...
  const [tooltip, setTooltip] = useState({ visible: false, x: 0, y: 0, dept: null })

  const handleMouseEnter = (dept) => {
    setTooltip(t => ({ ...t, visible: true, dept }))
  }
  const handleMouseMove = (e, dept) => {
    if (!hostRef.current) return
    const r = hostRef.current.getBoundingClientRect()
    setTooltip({ visible: true, x: e.clientX - r.left, y: e.clientY - r.top, dept })
  }
  ...
  const getDeptColor = (dept) => {
    if (partyFilter !== 'all') return partyById(partyFilter).color
    if (mode === 'participacion') {
      const intensity = Math.max(0.35, Math.min(1, dept.participacion / 84))
    ...
    return partyById(dept.leader).color
  }
  ...
  const sorted = [...departments]
    .filter(d => partyFilter === 'all' || d.pct[partyFilter] >= 0)
    .sort((a, b) => getDeptValue(b) - getDeptValue(a))

  return (
    <div style={{ display: 'grid', gridTemplateColumns: showLegend ? 'minmax(0,1.3fr) minmax(0,1fr)' : '1fr', gap: 30, alignItems: 'stretch' }}
      className="map-block">
      <div className="map-svg-host" ref={hostRef}>
        {tooltip.visible && tooltip.dept && (() => {
          ...
          return (
            <div className="map-tt show" style={{ left: tooltip.x, top: tooltip.y }}>
              ...
              <div className="tt-row"><span>ACTAS</span><span>{d.actas}/{d.mesas}</span></div>
            </div>
          )
        })()}

        <svg
          viewBox="0 0 612.67 692.763"
          preserveAspectRatio="xMidYMid meet"
          style={{ width: '100%', height: '100%', transform: `scale(${zoom})`, transformOrigin: 'center', transition: 'transform 0.28s cubic-bezier(0.22, 1, 0.36, 1)' }}
        >
          {departments.map(dept => {
            const lead = partyById(dept.leader)
            const d = DEPT_PATHS[dept.id]
            if (!d) return null
            return (
              <path
                key={dept.id}
                ...
                className={selectedDeptId === dept.id ? 'is-selected' : ''}
                style={{ cursor: 'pointer', stroke: 'var(--bg-2)', strokeWidth: 1.2, transition: 'opacity 0.2s, filter 0.2s' }}
      ...
      {showLegend && <div className="legend-list">
        {sorted.map(d => {
          const lead = partyById(d.leader)
          return (
            <button key={d.id} type="button" className={`legend-row${selectedDeptId === d.id ? ' is-active' : ''}`} onClick={() => onSelectDept?.(d)}>
              <span className="chip" style={{ background: partyFilter === 'all' ? lead.color : partyById(partyFilter).color }} />
              <span className="name">{d.name}</span>
              <span className="actas">{d.actas}/{d.mesas}</span>
              <span className="pct" style={{ color: partyFilter === 'all' ? lead.color : partyById(partyFilter).color }}>{getDeptValue(d).toFixed(1)}%</span>
            </button>
          )
        })}
      </div>}
