...
Tablas:
  distribucion_territorial → recinto_electoral → mesa_electoral
  usuario
  acta_oficial (mesa + usuario)
  voto_oficial (acta + usuario)
  auditoria_voto (voto + usuario)
  fallo_db (registro de fallos del cluster)
"""
...
class ActaOficial(Base):
    ...
    __table_args__ = (
        CheckConstraint("estado IN ('PENDIENTE','PROCESADO','OBSERVADO','RECHAZADO')",
...
class VotoOficial(Base):
    ...
    __table_args__ = (
        CheckConstraint("votos_validos = partido1 + partido2 + partido3 + partido4",
                        name="chk_votos_validos"),
        CheckConstraint("total_votos = votos_validos + votos_blancos + votos_nulos",
                        name="chk_total_votos"),
        CheckConstraint("partido1 >= 0 AND partido2 >= 0 AND partido3 >= 0 AND partido4 >= 0 "
